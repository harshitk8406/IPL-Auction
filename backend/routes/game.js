...
router.post('/create', authenticate, async (req, res) => {
  try {
    ...
    res.status(201).json({ game: { id: game.id, lobbyCode: game.lobbyCode, status: game.status, hostUserId: game.hostUserId } });
  } catch (err) {
...
router.post('/join', authenticate, async (req, res) => {
  try {
    ...
    res.json({ game: { id: game.id, lobbyCode: game.lobbyCode, status: game.status, hostUserId: game.hostUserId } });
  } catch (err) {
...
router.post('/:gameId/start', authenticate, async (req, res) => {
  try {
    const game = await Game.findByPk(req.params.gameId);
    if (!game) return res.status(404).json({ error: 'Game not found' });
    if (game.hostUserId !== req.user.id) return res.status(403).json({ error: 'Only host can start' });
    if (game.status !== 'waiting') return res.status(400).json({ error: 'Game already started' });

    // Shuffle all players for the pool
    const players = await Player.findAll({ attributes: ['id'] });
    const shuffled = players.map((p) => p.id).sort(() => Math.random() - 0.5);

    await game.update({ status: 'auction', playerPoolJson: shuffled });

    // Update auction state with shuffled pool
    const auctionState = await AuctionState.findOne({ where: { gameId: game.id } });
    if (auctionState) {
      await auctionState.update({ playerPoolJson: shuffled, status: 'nominating' });
    }
